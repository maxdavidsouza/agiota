'use client';
import { atualizarAgiota, carregarAgiota } from "@/app/lib/funcoes";
import { useState, useEffect } from "react";
import { Formik, Form, Field, ErrorMessage } from 'formik';
import { useRouter } from "next/navigation";
import { Row, Col, InputGroup, Form as BootstrapForm, Button } from 'react-bootstrap';
import InputMask from 'react-input-mask';
import styles from './UpdateAgiota.module.css';

export default function UpdateAgiota({ params }) {
  ...
  useEffect(() => {
    const loadForm = async () => {
      try {
        const response = await carregarAgiota(params.id);
        ...
        setErro("Erro ao carregar dados do agiota.");
      }
    };
  ...
  const handleSubmit = async (values) => {
    setErro("");
    try {
      await atualizarAgiota(params.id, values);
  ...
  return (
    <>
      ...
      <div className={styles.topBar}>
        <span className={styles.pageTitle}>Página de Edição de Agiotas</span>
      </div>
          ...
          {({ isSubmitting, errors, touched, values, handleChange }) => (
            <Form className="w-75">
              ...
              <Button type="submit" variant="primary" disabled={isSubmitting} className={styles.submitButton} style={{ marginTop: "0.5rem" }}>
                {isSubmitting ? 'Atualizando...' : 'Atualizar Agiota'}
              </Button>
            </Form>
          )}
        </Formik>
